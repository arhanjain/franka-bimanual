...
class EnvFrameFranka(Robot):
    ...
    def observation_features(self) -> dict[str, type | tuple[int, int, int]]:
        # EE pose per arm (pose-only; no gripper sensor) + any configured cameras.
        return {**self._arm_features, **self._camera_features}
    ...
    def send_action(self, action: RobotAction | BimanualAction) -> RobotAction:
        ...
        dict actually dispatched. The gripper field is accepted to match the sim
        LBM-ImplicitIK action space but is not actuated yet.
        """
