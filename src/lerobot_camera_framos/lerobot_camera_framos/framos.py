...
class FramosCamera(Camera):
    def __init__(self, config: FramosCameraConfig):
        ...
        # Request the color STREAM natively at the output size when one is set, so
        # there is no software resize on top (the requested size must be a mode
        # the device supports, e.g. 640x480 on the D415e). Falls back to the
        # configured color_width/height when no output size is given.
        self._stream_color_width = self._output_width
        self._stream_color_height = self._output_height
    ...
    def connect(self, warmup: bool = False) -> None:
        ...
        if self._config.enable_color:
            cfg.enable_stream(
                rs.stream.color,
                int(self._stream_color_width),
                int(self._stream_color_height),
        ...
        # Read intrinsics live from the negotiated color stream rather than
        # trusting the config default: factory intrinsics scale with the stream
        # resolution (we run 224x224 here, 640x480 in the envframe rig) and
        # differ per physical D415e, so a single hardcoded matrix is wrong for
        # one rig or one camera. Depth is aligned to color (align_to), so the
        # color intrinsics are the right ones for get_depth()'s deprojection.
        # Config intrinsic_matrix stays as a fallback if the read fails.
        try:
            ...
            logger.warning(
                "FRAMOS %s: could not read live color intrinsics; using config default",
                self._name,
            )
    ...
    def _fetch_color(self, timeout_ms: float, allow_stale: bool) -> np.ndarray:
        ...
        while True:
            ...
            # The color stream is requested natively at the output size (see
            # connect()), so this is normally a no-op. Kept as a safety net in
            # case the device snapped the requested mode to a different size.
            if arr.ndim == 3 and (
